...
func (r *postgresUserRepository) Search(ctx context.Context, q string) ([]domain.User, error) {
	log.Default().Println("NOT IMPLEMENTED: UserRepository.Search")
	return nil, nil
}
...
func (r *postgresUserRepository) CreateOrUpdate(ctx context.Context, u *domain.User) error {
	...
	query := `
		INSERT INTO users (id, username, name, auth_id)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT (username) DO NOTHING
		RETURNING id`
	return r.conn.QueryRow(ctx, query, u.ID, u.Username, u.Name, u.AuthID).Scan(&u.ID)
}
